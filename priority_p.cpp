...
int main()
{
    ...
    int bt_remaining[100]; // this will contain the values when the process will preempt
    ...
    while(completed < n) {
        ...
        if(max_ind == -1) curr_time++;
            ...
            if(bt_remaining[max_ind] == ps[max_ind].bt) {
                ps[max_ind].start_time = curr_time;
            }
            bt_remaining[max_ind]--;
            curr_time++;
            
            if(bt_remaining[max_ind] == 0) {
                // ps[max_ind].start_time = curr_time;
                ps[max_ind].ct = curr_time;
